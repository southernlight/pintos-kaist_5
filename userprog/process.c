...
struct aux_lazy_load {
  struct file *file;        // 해당 페이지의 파일
  off_t ofs;                // 파일에서 읽어야 할 오프셋
  uint32_t page_read_bytes; // 읽어야 할 바이트 수
  uint32_t page_zero_bytes; // 제로 패딩 바이트 수
};
...
tid_t process_fork(const char *name, struct intr_frame *if_ UNUSED) {
  ...
  struct intr_frame *f =
      (pg_round_up(rrsp()) -
       sizeof(
           struct intr_frame)); // 현재 쓰레드의 if_는 페이지 마지막에 붙어있다.
  memcpy(&curr->parent_if, f,
         sizeof(struct intr_frame)); // 1. 부모를 찾기 위해서 2. do_fork에
                                     // 전달해주기 위해서
  ...
  sema_down(&child->fork_sema); // 생성만 해놓고 자식 프로세스가 __do_fork에서
                                // fork_sema를 sema_up 해줄 때까지 대기
...
static void __do_fork(void *aux) {
  ...
  sema_up(&current->fork_sema); // fork 프로세스가 정상적으로 완료됐으므로 현재
                                // fork용 sema unblock
  process_init();
...
void process_exit(void) {
  ...
  sema_up(&curr->wait_sema); // 자식 프로세스가 종료될 때까지 대기하는 부모에게
                             // signal
...
static bool lazy_load_segment(struct page *page, void *aux) {
  ...
  /* TODO: This called when the first page fault occurs on address VA. */
  /* TODO: VA is available when calling this function. */
  struct aux_lazy_load *info = (struct aux_lazy_load *)aux;
  struct file *file = info->file; // file
  off_t ofs = info->ofs;          // filepage
