...
static bool less_awake_time_thread(const struct list_elem *a,
...
void thread_yield(void) {
  ...
  old_level = intr_disable();
  if (curr != idle_thread)
    list_insert_ordered(&ready_list, &curr->elem, greater_priority_thread,
                        NULL);
...
int thread_get_recent_cpu(void) {
  enum intr_level old_level = intr_disable();
  int recent_cpu =
      fp_to_int_round(mult_mixed(thread_current()->recent_cpu, 100));
  intr_set_level(old_level);
  return recent_cpu;
}
...
void mlfqs_calculate_priority(struct thread *t) {
  ...
  // 공식 적용
  t->priority =
      fp_to_int(add_mixed(div_mixed(t->recent_cpu, -4), PRI_MAX - t->nice * 2));
}
...
void mlfqs_calculate_recent_cpu(struct thread *t) {
  ...
  t->recent_cpu =
      add_mixed(mult_fp(div_fp(mult_mixed(load_avg, 2),
